...
- [ ] Add `proofRequirement` column to `packages/core/src/schema/lists.ts` (AC: 1)
  - [ ] Column: `proofRequirement: text()` — nullable, no default (null means no requirement / `'none'`)
  - [ ] Valid values: `'none'` | `'photo'` | `'watchMode'` | `'healthKit'` | `null`
  - [ ] Follow existing pattern: camelCase in schema, Drizzle generates snake_case DDL automatically

- [ ] Add `proofRequirement` column to `packages/core/src/schema/sections.ts` (AC: 1)
  - [ ] Column: `proofRequirement: text()` — nullable (null = inherit from parent list)
  - [ ] Same valid values as list-level: `'none'` | `'photo'` | `'watchMode'` | `'healthKit'` | `null`

- [ ] Add `proofMode` and `proofModeIsCustom` columns to `packages/core/src/schema/tasks.ts` (AC: 1, 2)
  - [ ] `proofMode: text()` — nullable (null = derived from inherited requirement; `'standard'` means no proof)
  - [ ] `proofModeIsCustom: boolean().default(false).notNull()` — true when the task's `proofMode` was explicitly set by the user, overriding the list/section default
  - [ ] NOTE: `assignedToUserId` already exists — do NOT add it again (committed in Story 5.2 migration `0009_task_assignment_strategies.sql`)

- [ ] Generate migration `packages/core/src/schema/migrations/0010_accountability_settings_cascade.sql` (AC: 1, 2)
  - [ ] Run `pnpm drizzle-kit generate` from `packages/core/` to produce the migration file
  - [ ] Migration must: ADD `proof_requirement` to `lists`, ADD `proof_requirement` to `sections`, ADD `proof_mode` to `tasks`, ADD `proof_mode_is_custom` to `tasks`
  - [ ] Commit generated SQL, updated `meta/_journal.json`, and `meta/0010_snapshot.json`

### Backend: API — extend `listSchema` and `sectionSchema` in `apps/api/src/routes/lists.ts`

- [ ] Add `proofRequirement` to `listSchema` in `apps/api/src/routes/lists.ts` (AC: 1)
  - [ ] `proofRequirement: z.enum(['none', 'photo', 'watchMode', 'healthKit']).nullable()`
  - [ ] Update `stubList()` to include `proofRequirement: null`
  - [ ] Existing tests continue to pass — `proofRequirement: null` is the safe default (additive, non-breaking)

- [ ] Add `proofRequirement` to `sectionSchema` in `apps/api/src/routes/lists.ts` (AC: 1)
  - [ ] `proofRequirement: z.enum(['none', 'photo', 'watchMode', 'healthKit']).nullable()`
  - [ ] Update any stub section fixtures to include `proofRequirement: null`

- [ ] Add `PATCH /v1/lists/{id}/accountability` endpoint in `apps/api/src/routes/lists.ts` (AC: 1)
  - [ ] Request body schema: `{ proofRequirement: z.enum(['none', 'photo', 'watchMode', 'healthKit']).nullable() }`
  - [ ] Response 200: `{ data: <full listSchema with updated proofRequirement> }`
  - [ ] Response 403: caller is not the list owner
  - [ ] Response 404: list not found
  - [ ] Response 422: invalid requirement value (Zod parse error)
  - [ ] Stub: return 200 with `stubList({ proofRequirement: body.proofRequirement })`; add `TODO(impl): verify ownership from JWT, update lists table via Drizzle, cascade to tasks where proofModeIsCustom = false`
  - [ ] Tag: `'Lists'`
  - [ ] Register BEFORE the parameterized `PATCH /v1/lists/{id}` route (same rule as `PATCH /v1/lists/{id}/settings` — specific before parameterized)
  - [ ] Use `@hono/zod-openapi` `createRoute` pattern — no untyped routes
  - [ ] Use `.js` extensions for all local imports

- [ ] Add `PATCH /v1/sections/{id}/accountability` endpoint in `apps/api/src/routes/lists.ts` (AC: 1)
  - [ ] Request body schema: `{ proofRequirement: z.enum(['none', 'photo', 'watchMode', 'healthKit']).nullable() }`
  - [ ] Response 200: `{ data: <sectionSchema with updated proofRequirement> }`
  - [ ] Response 403: caller is not a list owner
  - [ ] Response 404: section not found
  - [ ] Stub: return 200 with the patched section; add `TODO(impl): verify caller is list owner via list_members, update sections table, cascade to tasks in this section where proofModeIsCustom = false`
  - [ ] Tag: `'Lists'`
  - [ ] Register BEFORE `PATCH /v1/sections/{id}` if that route exists, otherwise add after list-level accountability route
  - [ ] Use `.js` extensions for all local imports

### Backend: API — extend `taskSchema` with `proofMode` and `proofModeIsCustom` in `apps/api/src/routes/tasks.ts`

- [ ] Add `proofModeIsCustom` to `taskSchema` in `apps/api/src/routes/tasks.ts` (AC: 2)
  - [ ] `proofModeIsCustom: z.boolean()` — false by default; true when this task's proofMode differs from its inherited list/section requirement
  - [ ] `proofMode` already exists in `currentTaskSchema` (for the Now card) but is NOT yet on the general `taskSchema` — add it there: `proofMode: z.enum(['standard', 'photo', 'watchMode', 'healthKit', 'calendarEvent'])`
  - [ ] Update `stubTask()` to include `proofMode: 'standard' as const` and `proofModeIsCustom: false`
  - [ ] This is additive/non-breaking: existing callers receive `proofMode: 'standard'` and `proofModeIsCustom: false` as defaults

- [ ] Add `PATCH /v1/tasks/{id}/proof-mode` endpoint in `apps/api/src/routes/tasks.ts` — per-task override (AC: 2)
  - [ ] Request body schema: `{ proofMode: z.enum(['standard', 'photo', 'watchMode', 'healthKit']) }` (no `calendarEvent` — that is read-only from calendar integration)
  - [ ] Response 200: `{ data: <taskSchema with updated proofMode and proofModeIsCustom: true> }`
  - [ ] Response 404: task not found
  - [ ] Stub: return 200 with `stubTask({ proofMode: body.proofMode, proofModeIsCustom: true })`; add `TODO(impl): set proofMode = body.proofMode, proofModeIsCustom = true in tasks table`
  - [ ] Tag: `'Tasks'`
  - [ ] Register BEFORE the parameterized `PATCH /v1/tasks/{id}` route
  - [ ] Use `.js` extensions for all local imports

### Flutter: Domain models — extend `TaskList`, `Section`, and `Task`

- [ ] Add `String? proofRequirement` to `apps/flutter/lib/features/lists/domain/task_list.dart` (AC: 1)
  - [ ] Nullable, no `@Default` needed — null means no requirement configured at list level
  - [ ] Valid values: `'none'` | `'photo'` | `'watchMode'` | `'healthKit'` | `null`
  - [ ] Regenerate `task_list.freezed.dart` — commit generated file

- [ ] Add `String? proofRequirement` to `apps/flutter/lib/features/lists/domain/section.dart` (AC: 1)
  - [ ] Nullable, no `@Default` needed — null means inherit from parent list
  - [ ] Regenerate `section.freezed.dart` — commit generated file

- [ ] Add `ProofMode proofMode` and `bool proofModeIsCustom` to `apps/flutter/lib/features/tasks/domain/task.dart` (AC: 1, 2)
  - [ ] `@Default(ProofMode.standard) ProofMode proofMode` — defaults to standard (no proof required)
  - [ ] `@Default(false) bool proofModeIsCustom` — true when this task has a user-set override
  - [ ] Import `ProofMode` from `'../../now/domain/proof_mode.dart'` — it already exists, do NOT create a duplicate
...
- [ ] Add `proofRequirement` to `apps/flutter/lib/features/lists/data/list_dto.dart` (AC: 1)
  - [ ] `@JsonKey(defaultValue: null) String? proofRequirement`
  - [ ] Extend `toDomain()` to pass `proofRequirement` through
  - [ ] Regenerate `list_dto.freezed.dart` and `list_dto.g.dart` — commit both

- [ ] Add `proofRequirement` to `apps/flutter/lib/features/lists/data/section_dto.dart` (AC: 1)
  - [ ] `@JsonKey(defaultValue: null) String? proofRequirement`
  - [ ] Extend `toDomain()` to pass `proofRequirement` through
  - [ ] Regenerate `section_dto.freezed.dart` and `section_dto.g.dart` — commit both

- [ ] Add `proofMode` and `proofModeIsCustom` to `apps/flutter/lib/features/tasks/data/task_dto.dart` (AC: 1, 2)
  - [ ] `@JsonKey(defaultValue: 'standard') String proofMode` — raw string from API
  - [ ] `@JsonKey(defaultValue: false) bool proofModeIsCustom`
  - [ ] Extend `toDomain()`: convert `proofMode` via `ProofMode.fromJson(proofMode)`, pass `proofModeIsCustom` through
  - [ ] Import `ProofMode` from the correct path: `'../../now/domain/proof_mode.dart'`
...
- [ ] Add `updateListAccountability` to `apps/flutter/lib/features/lists/data/lists_repository.dart` (AC: 1)
  - [ ] `Future<TaskList> updateListAccountability(String listId, String? proofRequirement)` — `PATCH /v1/lists/$listId/accountability`
  - [ ] Request body: `{ 'proofRequirement': proofRequirement }`
...
- [ ] Add `updateSectionAccountability` to `apps/flutter/lib/features/lists/data/sections_repository.dart` (AC: 1)
  - [ ] `Future<Section> updateSectionAccountability(String sectionId, String? proofRequirement)` — `PATCH /v1/sections/$sectionId/accountability`
  - [ ] Request body: `{ 'proofRequirement': proofRequirement }`
  - [ ] Parse `response.data!['data']` using `SectionDto.fromJson(...)` → `.toDomain()`
  - [ ] Regenerate `sections_repository.g.dart` if provider hash changes — commit

- [ ] Add `setTaskProofMode` to the tasks repository (or `SharingRepository`) — per-task override (AC: 2)
  - [ ] Create method in `apps/flutter/lib/features/tasks/data/tasks_repository.dart` (tasks domain operation, not sharing): `Future<Task> setTaskProofMode(String taskId, String proofMode)` — `PATCH /v1/tasks/$taskId/proof-mode`
  - [ ] Parse `response.data!['data']` using `TaskDto.fromJson(...)` → `.toDomain()`
...
- [ ] Extend `apps/flutter/lib/features/lists/presentation/list_settings_screen.dart` with a new "Accountability" section below the assignment strategy section (AC: 1)
  - [ ] Show four options as a `CupertinoListTile` radio-style list (same pattern as assignment strategy):
  ...
  - [ ] On selection change: calls `listsRepository.updateListAccountability(listId, newRequirement)` — show loading indicator; on success, `ref.invalidate(listsProvider)` so the list refreshes
  - [ ] Section header: `AppStrings.accountabilitySettingsLabel`
  - [ ] Error state: show `AppStrings.accountabilityUpdateError` on failure
  - [ ] Background: `colors.surfacePrimary` (already set on the screen)
  - [ ] `minimumSize: const Size(44, 44)` on any `CupertinoButton`

### Flutter: Section widget — section-level accountability setting (AC: 1)

- [ ] Add an accountability picker to `apps/flutter/lib/features/lists/presentation/widgets/section_widget.dart` (AC: 1)
  - [ ] Section settings currently does not exist — add a trailing settings icon (`CupertinoIcons.ellipsis_circle`) to the section header row that opens a `CupertinoActionSheet`
  - [ ] Action sheet options: Set proof requirement (opens bottom sheet or inline picker), Rename, Delete
  - [ ] For "Set proof requirement": show the same four options (None / Photo / Watch Mode / HealthKit) as `CupertinoActionSheet` actions or a `showCupertinoModalPopup` picker
  - [ ] On selection: call `sectionsRepository.updateSectionAccountability(section.id, newRequirement)` → `ref.invalidate(sectionsProvider(section.listId))`
  - [ ] Only show the settings icon if the current user is a list owner (check `ListMember.role == 'owner'` from `listMembersProvider`) — fallback: show for all users in v1 since ownership is not yet wired in all views
  - [ ] Error state: `AppStrings.accountabilityUpdateError`

### Flutter: Task row — show inherited proof requirement label (AC: 1)

- [ ] Update `apps/flutter/lib/features/tasks/presentation/widgets/task_row.dart` to show the proof requirement label when `task.proofMode != ProofMode.standard` (AC: 1)
  - [ ] Show a `ProofModeIndicator` widget (already exists at `apps/flutter/lib/features/now/presentation/widgets/proof_mode_indicator.dart`) or a small inline label
  - [ ] Use `colors.textSecondary`, SF Pro 13pt — consistent with attribution chip style from Story 5.3
  - [ ] When `task.proofModeIsCustom == true`, show the indicator with a "custom" suffix badge (e.g., `AppStrings.accountabilityCustomBadge = 'Custom'`) so the user sees it differs from the section default
  - [ ] When `task.proofModeIsCustom == false` and `task.proofMode != ProofMode.standard`, show the indicator with label only (e.g., "Photo proof") with no badge
  - [ ] Display-only in v1 — tapping the indicator does nothing in the task row (tapping the full row opens `TaskEditInline`)

### Flutter: Task edit — per-task proof mode override (AC: 2)

- [ ] Add a proof mode picker row to `apps/flutter/lib/features/tasks/presentation/widgets/task_edit_inline.dart` (AC: 2)
  - [ ] Add a new row below the energy requirement field: "Proof" → shows current `task.proofMode` display name
  - [ ] Tapping opens a `CupertinoActionSheet` with options: Standard (no proof), Photo proof, Watch Mode, HealthKit
  - [ ] On selection: call `tasksRepository.setTaskProofMode(task.id, selectedMode)` → `ref.invalidate(tasksProvider(task.listId))`
  - [ ] When `task.proofModeIsCustom == true`, show the "Custom" badge next to the current value so the user knows they have an override
  - [ ] When the user selects "Standard" AND the list/section has a non-null requirement, add a note: `AppStrings.accountabilityOverrideToStandardNote` (e.g., `'This overrides the section default'`)
  - [ ] Error state: show inline error text `AppStrings.accountabilityUpdateError` on failure; do not dismiss the picker

### Flutter: l10n strings (AC: 1, 2)

- [ ] Add to `apps/flutter/lib/core/l10n/strings.dart` under a new `// ── Accountability settings cascade (FR20) ──` section (AC: 1, 2)
  - [ ] `static const String accountabilitySettingsLabel = 'Proof requirement';` — section header in List Settings
  - [ ] `static const String accountabilityNone = 'None';` — no proof required
  - [ ] `static const String accountabilityPhoto = 'Photo proof';` — photo verification
  - [ ] `static const String accountabilityWatchMode = 'Watch Mode';` — passive camera session
  - [ ] `static const String accountabilityHealthKit = 'HealthKit';` — HealthKit data verification
  ...
  - [ ] `static const String accountabilityCustomBadge = 'Custom';` — shown on task row when proofModeIsCustom = true
  - [ ] `static const String accountabilityInheritedLabel = 'Inherited';` — for accessibility labels on inherited indicators
  - [ ] `static const String accountabilityUpdateError = 'Could not update proof requirement. Please try again.';`
  - [ ] `static const String accountabilityOverrideToStandardNote = 'This overrides the section default.';` — shown when user overrides to None/standard
  - [ ] NOTE: `AppStrings.nowCardProofPhoto`, `AppStrings.nowCardProofWatchMode`, `AppStrings.nowCardProofHealthKit` already exist (used by `ProofModeIndicator`) — do NOT duplicate them; reuse where appropriate

### Tests

- [ ] Widget test for `ListSettingsScreen` accountability section in `apps/flutter/test/features/lists/list_settings_screen_test.dart` (AC: 1)
  - [ ] Extend existing `list_settings_screen_test.dart` (created in Story 5.2)
  - [ ] Test: accountability section renders all four options (None, Photo proof, Watch Mode, HealthKit)
  - [ ] Test: tapping "Photo proof" calls `updateListAccountability(listId, 'photo')` — stub `listsRepository` with a `mocktail` mock
  - [ ] Test: currently selected option shows a checkmark (consistent with assignment strategy UI pattern)
  - [ ] Override `listsRepositoryProvider` AND `sharingRepositoryProvider` with stub notifiers — same `ProviderContainer` pattern as Story 5.2 tests

- [ ] Widget test for `task_row.dart` proof mode label in `apps/flutter/test/features/tasks/task_row_test.dart` (AC: 1, 2)
  - [ ] Create or extend `task_row_test.dart`
  - [ ] Test: when `task.proofMode = ProofMode.photo` and `task.proofModeIsCustom = false`, the "Photo proof" label renders (no "Custom" badge)
  - [ ] Test: when `task.proofMode = ProofMode.photo` and `task.proofModeIsCustom = true`, both the label and `AppStrings.accountabilityCustomBadge` render
  - [ ] Test: when `task.proofMode = ProofMode.standard`, no proof indicator is rendered
  - [ ] Wrap in `MaterialApp` with `OnTaskTheme` to resolve `OnTaskColors` extension

- [ ] Unit test for `TaskDto.fromJson` handles `proofMode` and `proofModeIsCustom` in `apps/flutter/test/features/tasks/task_dto_test.dart` (AC: 1, 2)
  - [ ] Extend existing `task_dto_test.dart` (created in Story 5.2, extended in Story 5.3)
  - [ ] JSON with `proofMode: 'photo'` parses to `ProofMode.photo`
  - [ ] JSON with `proofMode: 'watchMode'` parses to `ProofMode.watchMode`
  - [ ] JSON WITHOUT `proofMode` (old API stub) parses to `ProofMode.standard` via `@JsonKey(defaultValue: 'standard')`
  - [ ] JSON with `proofModeIsCustom: true` parses correctly; WITHOUT the field defaults to `false`

- [ ] Unit test for `ListDto.fromJson` handles `proofRequirement` in `apps/flutter/test/features/lists/list_dto_test.dart` (AC: 1)
  - [ ] Extend existing `list_dto_test.dart` (created in Story 5.1)
  - [ ] JSON with `proofRequirement: 'photo'` parses correctly
  - [ ] JSON WITHOUT `proofRequirement` parses to `null` via `@JsonKey(defaultValue: null)`

- [ ] Unit test for `SectionDto.fromJson` handles `proofRequirement` in `apps/flutter/test/features/lists/section_dto_test.dart` (AC: 1)
  - [ ] Create or extend `section_dto_test.dart`
  - [ ] JSON with `proofRequirement: 'watchMode'` parses correctly
  - [ ] JSON WITHOUT `proofRequirement` parses to `null`
