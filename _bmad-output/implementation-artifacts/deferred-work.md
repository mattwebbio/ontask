...
- **Manually faked `.g.dart` hash causes `build_runner` regeneration** — `notification_handler.g.dart` uses a hard-coded fake hash `a1b2c3d4e5f6...` instead of one generated by `build_runner`. Any developer who runs `build_runner build` locally will see the file regenerated with a different hash, producing a dirty working tree. Documented project convention (CI doesn't run build_runner); acceptable for now but may confuse contributors. [`apps/flutter/lib/features/notifications/presentation/notification_handler.g.dart:54`]
...
## Deferred from: code review of 13-5-testflight-first-build (2026-04-02)

- **Gemfile has no version constraint on fastlane** — `gem "fastlane"` is unpinned in `apps/flutter/fastlane/Gemfile`; the lock file pins it for now but a future `bundle update` could silently pull a breaking version. Consider pinning with `gem "fastlane", "~> 2.232"`.
- **`.tool-versions` Ruby pin only effective when cd'd into fastlane/ directory** — `apps/flutter/fastlane/.tool-versions` pins `ruby 3.4.1` but the root `.tool-versions` has no Ruby entry. Developers running `bundle install` from outside `apps/flutter/fastlane/` may use a mismatched Ruby version. Document in `APP_STORE_SETUP.md` that `bundle install` must be run from `apps/flutter/fastlane/`.
- **sprint-status shows 13-4 as `in-progress` despite being merged to main** — Pre-existing staleness on main; `_bmad-output/implementation-artifacts/sprint-status.yaml` was not updated when story 13-4 was closed.
- **Tasks 3 & 4 (fastlane beta run and device verification) have no verifiable artifact** — Marked `[x]` done but documented as "done manually" with no TestFlight build URL, build number, or device test log. Acceptable for manual execution steps; note for future stories to capture build evidence.
- **No `.gitattributes` merge strategy defined for `project.pbxproj`** — `apps/flutter/ios/Runner.xcodeproj/project.pbxproj` is merge-conflict-prone; concurrent branches could silently corrupt it. Consider adding `*.pbxproj merge=union` to `.gitattributes`.
